...
def affiche(xyz,indS,indU,indD,data,states = np.zeros(10),error = np.zeros(10),mode = None,filename = False,**kwargs):
...
def affiche_chiffre(liste_chiffre,indS,indU,data,list_error = [], mode = None,filename = False,**kwargs):
...
def W_in(N,M):
    ...
    return sparse.csr_matrix(np.transpose(Win))
