...
class RecurrentNetwork(object):
    ...
    * dtype: floating point precision (default: np.float32)
    """
    def __init__(self,W_in,W_rec,W_out, Ni=2, N=800, No=1, tau=10.0, g=1.5, pc=0.1, Io=0.001, delta=1.0, P_plastic=0.6, dtype=np.float32):
        # Copy the parameters
        self.W_in = W_in
        self.W_rec = W_rec
        self.W_out = W_out
    ...
    def simulate(self, stimulus, noise=True, trajectory=np.array([]), learn_start=-1, learn_stop=-1, learn_readout=False, verbose=True,value_loss = False):
        ...
        * 'value_loss' : definis if the loss should be return (default: False)
        ...
        for t in tqdm(range(duration),mininterval = 0.5):
